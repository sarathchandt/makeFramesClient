...
function adminProducerList() {
    ...
    function takeUser() {
        axios.post(`${UARL}takeArtistByRegex`, { name: search ,from:'producer' }).then(res => {
    ...
    useEffect(() => {
        axios.get(`${UARL}fetchDetailsOfProducer`, { headers }).then(res => {
            setTableDetails(res.data)
        ...
        <div className='p-5  container-fluid'>
            <div className="row">
                <div className="col-12 d-flex justify-content-end mt-4  ">
                    <input type="text" className='m-3 email w-max border-darkGreen  border-4 rounded-lg bg-white text-black ' value={search} placeholder='Search Users' onChange={(e)=>{setSearch(e.target.value)}}  />
